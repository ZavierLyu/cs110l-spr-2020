...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_path_str = format!("/proc/{pid}/fd/{fd}");
        let fd_path = Path::new(&fd_path_str);
        let slink_path = fs::read_link(fd_path).ok()?;
        let name = Self::path_to_name(slink_path.to_str()?);

        let fd_info_path_str = format!("/proc/{pid}/fdinfo/{fd}");
        let content = fs::read_to_string(Path::new(&fd_info_path_str)).ok()?;
        let access = Self::parse_access_mode(&content)?;
        let cursor = Self::parse_cursor(&content)?;
        
        Some(OpenFile::new(name, cursor, access))
    }
